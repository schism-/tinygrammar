...
CSGTree::OpNode* CSGTree::Sum(CSGTree::Tree* tree, CSGTree::Node* a, CSGTree::Node* b){
...
CSGTree::OpNode* CSGTree::XOR(CSGTree::Tree* tree, CSGTree::Node* a, CSGTree::Node* b){
    vector<polygon2r> a_polys = make_vector(a->content->shapes, [&](AnimatedShape* shape){return shape->poly;});
    vector<polygon2r> b_polys = make_vector(b->content->shapes, [&](AnimatedShape* shape){return shape->poly;});
...
void CSGTree::UpdateLeafNode(CSGTree::Tree* tree, CSGTree::LeafNode* a, Animator anim){
    auto new_shapes = anim(a->content->shapes, rule_params());
    ...
    while (cur_node != nullptr){
        switch (cur_node->op_type) {
            case union_op:
            {
                auto temp = CSGTree::Union(tree, cur_node->child_left, cur_node->child_right);
                cur_node->content = temp->content;
                break;
            }
            case intersection_op:
            {
                auto temp = CSGTree::Intersection(tree, cur_node->child_left, cur_node->child_right);
                cur_node->content = temp->content;
                break;
            }
            case difference_op:
            {
                auto temp = CSGTree::Difference(tree, cur_node->child_left, cur_node->child_right);
                cur_node->content = temp->content;
                break;
            }
            default:
